...
function getCityImagePath(cityName?: string) {
  if (!cityName) return '/tehran.png';

  const name = cityName.toLowerCase();

  if (name.includes('shiraz')) return '/shirazHero.png';
  if (name.includes('isfahan') || name.includes('esfahan')) return '/isfahan.png';
  if (name.includes('kish')) return '/kish.png';
  if (name.includes('mashhad') || name.includes('mashad')) return '/mashad.png';
  if (name.includes('tabriz')) return '/tabriz.png';
...
export default async function HotelsPage({ searchParams }: HotelsPageProps) {
  ...
  // ✅ فقط همین بخش اضافه شد: ساخت رشته کوئری
  ...
  return (
// پدینگ حذف شد و مارجین منفی (-mt-16) اضافه شد
<div className="flex flex-col w-full min-h-screen bg-gray-50 -mt-16">

      {/* 🌟 بخش هیرو: اعمال گرادیان جدید برای محو شدن در پایین 🌟 */}
      <section className="relative w-full h-[260px] lg:h-[380px] shadow-sm">
        <div className="absolute inset-0 z-0">
          ...
          />
          {/* تغییر رنگ پایین گرادیان به خاکستری پس زمینه صفحه */}
          <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-gray-50"></div>
        </div>

        <div className="relative z-10 container-main h-full flex flex-col justify-between py-6 lg:py-10">
          {/* 🌟 تغییر سایز و موقعیت اسم شهر 🌟 */}
              ...
              <div className="flex flex-col gap-1.5 bg-black/20 backdrop-blur-md border border-white/10 rounded-2xl px-4 py-3 shadow-sm text-xs md:text-sm text-white/90 w-fit font-light tracking-wide">
                {checkIn && (
                  <div className="flex items-center gap-2">
                    <span className="opacity-70 w-12">In:</span>
                ...
                {checkOut && (
                  <div className="flex items-center gap-2">
                    <span className="opacity-70 w-12">Out:</span>
                ...
                {adults && (
                  <div className="flex items-center gap-2">
                    <span className="opacity-70 w-12">Guests:</span>
      ...
      <div className="container-main flex flex-col mt-4 lg:mt-8">
        {/* مودال موبایل */}
        ...
        <div className="fixed inset-0 z-[100] hidden peer-checked:flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity">
          ...
          <div className="relative z-10 w-full max-w-md bg-white sm:rounded-2xl rounded-t-2xl shadow-xl overflow-hidden p-4 pb-8 sm:pb-4 animate-in slide-in-from-bottom-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <label
                htmlFor="mobile-search-modal"
                className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-full w-8 h-8 flex items-center justify-center font-bold text-xl"
              >
                &times;
              </label>
            </div>
